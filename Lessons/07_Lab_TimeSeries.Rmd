...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #separate one column into multiple. like pivot wider but can delineate 
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
...
plot_rainfall <- ggplot(Rainfall_BR_processed) +
...
f_month <- month(first(Rainfall_BR_processed$Date))
f_year <- year(first(Rainfall_BR_processed$Date))
rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, start = c(f_year, f_month), frequency = 12)
...
decomposition_rainfall <- stl(rainfall_ts, s.window = "periodic")
plot(decomposition_rainfall)
...
DataFrame_Rainfall_Components <- as.data.frame(decomposition_rainfall$time.series[,1:3])


# Exercise 10: Visualize how the trend maps onto the data for both series

DataFrame_Rainfall_Components <- mutate(DataFrame_Rainfall_Components,
        Observed = BRA_complete$Rainfall,     
        Date = BRA_complete$Date)


Visualize <- ggplot(DataFrame_Rainfall_Components) +
