...
`>`
`>=`
`&` multiple conditions usd if both are true ( is an and statement)
`|` multiple conditions used if not all are tru (is an or statmennt)
...
summary(NTL.phys.data.PeterPaul1)
...
datayear4 <- filter(NTL.phys.data, year4 == 1999)
# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
yeartuesdaylake <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 > 1990 & year4 <= 1999)
