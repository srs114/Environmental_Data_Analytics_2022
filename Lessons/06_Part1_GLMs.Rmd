...
Function lm standsfor linear model
...
summary(irradiance.regression)     #estimate standard deviation in output shows mx+b
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #par dividing plot screen into 4 cells - mar dictating size
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #get linear regression line on plot
  scale_y_log10() + #how to show log transformed
  geom_point() 
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") #se gets rid of confidence interval on line
print(irradiancebydepth2)
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit()
PeterPaulCorr <- cor(PeterPaulnutrients) #matrix of correltaions between selected values
...
step(TPAIC) #gauging what independent values are explanatory. Lower AIC is better. Step shows you what will happen to AIC if you remove an independent value
