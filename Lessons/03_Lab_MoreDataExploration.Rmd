...
```{r}
high <- c(58, 41, 42, 45, 35, 42,48 , 51, 55, 66)
low <- c(30, 24, 19, 25, 21, 18, 22, 24, 29, 43)
```
...
```{r}
lowc <- c(-1, -4, -7, -4, -6, -8, -6, -4, -2, 6)
highc <- c(14, 5, 6, 7, 2, 6, 9, 11, 13, 19)

lowcfunction <- function(x){
  (x-32)*.556
}
lowcel <- lowcfunction(low)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
temperature_dataframe <- data.frame(high, highc, low, lowc)
names(temperature_dataframe) <- c("highF","HighC","LowF", "LowC"); View(temperature_dataframe) 
```
...
```{r}
summary(temperature_dataframe)
sd(temperature_dataframe$highF)
```
